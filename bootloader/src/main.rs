...
pub extern fn entry(boot_disk_id: u8, bootloader_size: u32) -> ! {
    ...
    let (kernel_entry, kernel_stack, new_cr3) = setup_kernel(boot_disk_id, bootloader_size);
...
/// Returns (kernel entry vaddr, kernel stack vaddr, new cr3)
fn setup_kernel(boot_disk_id: u8, bootloader_size: u32) -> (u32, u32, u32) {
    ...
    kernel_elf.for_segment(|vaddr, size, init_bytes, flags| {
        println!("Mapping kernel segment {:#09x} {:#09x} {:03b}", vaddr, size, flags);
    ...
    println!("Kernel entry @{:#x}, Page directory @{:#x}", kernel_entry, new_cr3);

    (kernel_entry, KERNEL_STACK_ADDR, new_cr3)
}
