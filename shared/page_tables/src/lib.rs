...
pub trait PhysMem {
    ...
    fn allocate_zeroed_phys_mem(&mut self, layout: Layout) -> Option<PhysAddr> {
        ...
        unsafe {
            ...
            let virt_addr = self.translate_phys(phys_addr, layout.size())?;
            // Zero it out
...
pub struct PageDirectory<'a, P: PhysMem> {
...
impl<'a, P: PhysMem> PageDirectory<'a, P> {
    // Creates a new empty page table
    pub fn new(phys_mem: &'a mut P) -> Option<Self> {
    ...
    pub unsafe fn from_cr3(phys_mem: &'a mut P, cr3: u32) -> Self {
        // We mask off the lower 12 bits of cr3 to get the address of the page directory
        PageDirectory {
            directory: PhysAddr(cr3 & !0xfff),
            phys_mem
        }
    }
    ...
    pub unsafe fn map(&mut self, virt_addr: VirtAddr, size: u32, write: bool, user: bool)
        -> Option<()> {
        self.map_internal(virt_addr, size, write, user, None::<fn(usize) -> u8>)
    }
    ...
    /// with its offset.
    pub unsafe fn map_init<F>(&mut self, virt_addr: VirtAddr, size: u32, write: bool,
        user: bool, init: F) -> Option<()>
        where F: Fn(usize) -> u8 {
        self.map_internal(virt_addr, size, write, user, Some(init))
    }
    ...
    unsafe fn map_internal<F>(&mut self, virt_addr: VirtAddr, size: u32, write: bool,
        ...
        for page in first_addr_page..=last_addr_page {
            ...
            let physical_page = self.phys_mem.allocate_phys_mem(page_layout)?;
            ...
            if let Some(init_bytes) = &init {
                ...
                let page_ptr = self.phys_mem.translate_phys(physical_page, 4096)?;
            ...
            self.map_raw(VirtAddr(page << 12), raw_page_table_entry, false)?;
        }
    ...
    pub unsafe fn map_raw(&mut self, virt_addr: VirtAddr, raw: u32, update: bool) -> Option<()> {
        ...
        let directory_entry_vaddr = self.phys_mem.translate_phys(directory_entry_paddr, 4)?;
        ...
        if (directory_entry & PAGE_ENTRY_PRESENT) == 0 {
            ...
            let new_table = self.phys_mem.allocate_zeroed_phys_mem(table_layout)?;
        ...
        // Translate it into a virtual address
        let table_entry_vaddr = self.phys_mem.translate_phys(table_entry_paddr, 4)?;
